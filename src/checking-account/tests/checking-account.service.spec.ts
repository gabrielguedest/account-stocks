...
describe('CheckingAccountService', () => {
  ...
  test('should make an account code', async () => {
    let codeChecks = 0;
    const checkCodeFn = jest
      .spyOn(checkingAccountRepository, 'checkIfCodeIsAvailable')
  ...
  test('should create a checking account', async () => {
    ...
    const createCheckingAccountFn = jest
      .spyOn(checkingAccountRepository, 'createCheckingAccount')
      .mockResolvedValue({
        code: '123456',
        balance: 0,
        createdAt: new Date(),
  ...
  describe('withdraw', () => {
    test('should withdraw from an account', async () => {
      ...
      const getCheckingAccountFn = jest
        .spyOn(checkingAccountRepository, 'getCheckingAccount')
        .mockResolvedValue(account);
      const updateBalanceFn = jest
        .spyOn(checkingAccountRepository, 'updateBalance')
